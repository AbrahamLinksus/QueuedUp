  ...
  return (
    <div className="select-none">
      ...
      >
        ...
        <span className="flex-1 text-[12px] font-medium capitalize tracking-[0.5px] text-foreground/70">
          {name.replace(/-/g, " ")}
        </span>
        ...
        {done > 0 && (
          <div className="h-1.5 w-12 shrink-0 overflow-hidden rounded-full bg-foreground/10">
            <div
      ...
      {open && (
        <div className="ml-3 border-l-[1.5px] border-foreground/10 pl-3">
          {problems.map((p) => (
            <ProblemRow key={p.id} p={p} loggedUrls={loggedUrls} />
          ))}
        </div>
      )}
    </div>
...
export function TopicSection({
  ...
  const general = problems.filter((p) => !p.subtopic);
  const subtopicMap = new Map<string, SheetRow[]>();
  for (const p of problems) {
    if (!p.subtopic) continue;
    const bucket = subtopicMap.get(p.subtopic) ?? [];
    bucket.push(p);
    subtopicMap.set(p.subtopic, bucket);
  }
    ...
    <div className="select-none">
      ...
      {open && (
        <div className="ml-3 border-l-[1.5px] border-foreground/15 pl-3">
          {/* General problems (no subtopic) */}
          {general.map((p) => (
            <ProblemRow key={p.id} p={p} loggedUrls={loggedUrls} />
          ))}

          {/* Subtopic sub-folders */}
          {[...subtopicMap.entries()].map(([name, subs]) => (
            <SubtopicFolder key={name} name={name} problems={subs} loggedUrls={loggedUrls} />
          ))}
        </div>
      )}
    </div>
